...
- 16 curated presets, deliberately muted: these sit behind real content as dots and edges, where a saturated neon reads as an error state rather than as a category. Custom colour via a native swatch plus a validated hex field that accepts `#abc`, `ABC` and `#4a7ab5` and rejects everything else rather than guessing.
- Foreground contrast is derived, not fixed. The luminance threshold is the real WCAG crossover at 0.179, found by a test: an earlier 0.45 put white text on mid-tan, which measures about 2.7:1 where black measures 7.8:1.
- Zod widened from an enum to a refined string, so a malformed colour is still rejected at the boundary rather than stored and rendered as nothing.

**Two functional bugs found by authenticated browser QA, both fixed:**
- "Add Task" in the top bar did nothing when you were already on /tasks. `openCreateOnMount` feeds `useState`, which is an INITIALIZER and runs once; `router.push("/tasks?new=1")` from /tasks is a soft navigation, so the prop stayed true and the form never opened. Now adjusted during render, React's documented answer for state that follows a prop, and `?new=1` is spent once honoured so pressing again is a real change.
- Escape inside the inline subtask composer closed the whole task. `DetailPanel` listens on `document`, so a React `stopPropagation` never reaches it. The panel now ignores an Escape whose `defaultPrevented` is already set, which is the signal that travels with the event. First Escape cancels the composer, second closes the panel: both measured in a browser.

- Verification: typecheck clean, lint clean, 737/737 unit tests pass, `pnpm build` compiles. Authenticated browser sweep of 13 pages at 1440x900, 390x844 and 430x932: 39 loads, 0px horizontal overflow on every one, zero console, page or network errors. Onboarding measured across all three steps at all three viewports: 0px height change, 0px width change, 0px progress shift, 0px action shift.

### 2026-08-27: Inline subtasks, calendar quick-add, onboarding shell, Daily Rhythm
- Subtask composer is revealed by "+ Add subtask" instead of sitting permanently open with its own Add button, which read as an unfinished row competing with the real steps above it. Enter saves and keeps it open and focused, because breaking a task down means typing several steps in a row and re-opening between each would put a click in the way. Escape cancels, and stops the event so it does not travel on and close the whole detail panel: cancelling a step is not cancelling the task. Blur on an empty composer closes it; a typed-but-unsaved value is kept rather than thrown away.
- Calendar quick-add: the whole empty area of a day is now the target. It was a 20px `+` that only appeared on hover, so on a phone it was invisible and unreachable, and on a desktop it asked for a precise click on the smallest thing in the cell. The cell button sits at `z-0` with task chips and the overflow control at `z-10`, so clicking a task still edits that task rather than creating a second one on the same day. The `+` is now decoration rather than a control, which keeps it to one tab stop per day instead of two. The clicked date passes through as a literal string, never a `Date`, so it cannot land a day away.
- Onboarding modal: all three steps now occupy ONE grid cell with only the active one visible, so the shell is as tall as the tallest step and the progress bar and buttons stay put when Continue is pressed. The old `min-h-[9rem]` was a guess that fitted step one and was overshot by the other two, which is what made it jump. Height comes from content, so a narrow screen that wraps grows the cell rather than clipping it. Inactive steps carry `aria-hidden` and `inert`, so only the visible step is reachable.
- Daily Rhythm: the two times were full-width fields, so a five-character value stretched across the card and read as a text box waiting for a sentence. Capped at the width the value needs and paired into a row from `sm:` up, stacked on a phone. The two help paragraphs were merged into one rather than left duplicated, and it now says these times are separate from the dates on a task.
- Daily Inspiration presentation only: `hyphens-none` so a long book name is not broken across lines on a narrow phone, `text-pretty` against a one-word last line, and slightly looser spacing between quote and attribution. Provider, resolver and payload untouched.
- Automation contracts verified unchanged: `deadline:{taskId}:{dueAtIso}` still keyed on a real instant, Morning still carries `dailyInspiration`, Evening still carries none, and `db/schema/settings.ts` is not in the diff so the Daily Rhythm schedule fields are untouched.
- Verification: typecheck clean, lint clean, 711/711 unit tests pass, `pnpm build` compiles. Public-page browser sweep re-run after the changes: 24 combinations across 1440x900, 390x844 and 430x932 in light and dark, 0px horizontal overflow everywhere, zero console or page errors, zero iOS zoom risk.
- NOT done and not started: expanded icon catalog, expanded/custom colour picker. Authenticated browser QA remains blocked for want of a test session.

### 2026-08-27: Smart date field, task row hierarchy, forgot-password target
- Native `<input type="date">`, `type="time"` and `type="datetime-local"` are gone from the task form, replaced by `components/ui/date-field.tsx`. The native control rendered different browser chrome everywhere, looked like raw HTML against the rest of GoHa, and made the two commonest answers (today, tomorrow) take a trip through a calendar. The new field offers those as one tap and keeps a real calendar for everything else, with arrow-key navigation, Escape to dismiss and a single roving tabstop so keyboard users do not tab through 42 day buttons to leave the field.
- Dates stay "YYYY-MM-DD" strings end to end and `today` is passed in already resolved from the SAVED timezone. A `Date` here carries a time and a zone, and the moment one is parsed in the browser's zone instead of the user's, the day shifts. Presets read the saved `weekStartsOn`, so "this week" is the user's week.
- Start time removed from the task form per the date-first direction. The `scheduled_time` COLUMN is untouched and any existing value is submitted straight back: dropping a control must not silently erase data entered under an earlier build.
- Due is now chosen as a date but still STORED as an instant, because deadline automation depends on it: the dedupe key is `deadline:{taskId}:{dueAtIso}` and the reminder computes `minutesUntil` from it. An existing task keeps whatever hour it had; a task given a due date for the first time gets end of local day, which is what "due on the 29th" means to a person. No migration, no contract change.
- Task row hierarchy reworked. Priority, goal and status were all chips at the same size, dates were `font-mono` with a full year, and priority and goal shared the same flag icon, so nothing led. Now: the title leads, the life-area colour stays on the leading edge, and one quiet secondary line carries the rest. Priority speaks only when high or urgent, the status badge is dropped for plain "To Do", the goal has its own icon, dates read as "Today" / "Tomorrow" / a real date, an overdue date is the one thing coloured, and subtask progress is a filled track rather than arithmetic.
- The forgot-password link was an 11px link in a 14px box, under the 44px target the spec mandates and easy to miss beside a red error. Measured in a real browser: 14px before, 135x44 after, against a 358x44 Sign in button, so it is comfortable to tap and still plainly secondary.
- Verification: typecheck clean, lint clean, 687/687 unit tests pass, `pnpm build` compiles. Real-browser sweep of the four public pages at 1440x900, 390x844 and 430x932 in light and dark: 24 combinations, 0px horizontal overflow everywhere, zero console or page errors, zero iOS zoom risk, zero fields under 44px on mobile.

### 2026-08-27: ZenQuotes as the active quote provider
- QuoteSlate removed from the runtime chain. Leaving it first cost a guaranteed failed request before every quote fetch: its deployment answers any non-browser client with HTTP 429 and a Vercel JS challenge, so on a server it can never succeed, and that latency was being spent on a known-impossible outcome. ZenQuotes is now the sole active quote provider for V1.
- `fetchQuoteSlate` is kept but DORMANT, absent from `QUOTE_PROVIDERS` and called by nothing at runtime. It is one self-contained function with its own schema and is still covered by tests, so re-enabling it if the challenge is ever lifted is a one-line change rather than a rewrite.
- Unchanged: 6s timeout, Zod validation, the 3 attempt ceiling, our own max-length check (ZenQuotes has no `maxLength` parameter, so length is enforced here by reject-and-retry rather than at the provider), 30 day duplicate avoidance, the curated local fallback, and bible-api.com for scripture.
- ZenQuotes requires attribution on its free tier, so the Today card now shows a small credit linking to zenquotes.io, driven by a `PROVIDER_CREDIT` map keyed on provider. Kept quiet on purpose: tertiary label at footnote size, placed after the author rather than competing with it. Nothing is shown for bible-api (the World English Bible is public domain) or for the curated pool.
- The credit is NOT added to the notification body. `dailyInspiration.provider` already travels in the payload, so n8n can decide for itself if it ever needs to; no workflow or prompt change is required.
- Verification: typecheck clean, lint clean, 660/660 unit tests pass, `pnpm build` compiles.

### 2026-08-27: Quote provider swap and Sabbath consistency fix
- QuoteGarden is retired (it answers HTTP 503, service suspended). QuoteSlate is the new primary, asked directly for what we want with `maxLength=160&tags=motivation,inspiration` so a usable quote arrives on the first attempt instead of burning retries. Serializes as `provider: "quoteslate"`.
- QuoteSlate is NOT reachable from a server. Its deployment sits behind Vercel Attack Challenge Mode, which answers any non-browser client with HTTP 429 and an HTML interstitial only a JavaScript-executing browser can clear. Verified with a single clean server request after a 20 second cooldown, and separately confirmed a real browser gets 200 and valid JSON from the identical URL. GoHa calls this from a Server Component and from the worker, so neither can clear it.
- ZenQuotes was therefore added behind QuoteSlate in an ordered chain, verified answering 200 with JSON server-side. Without it every quote day would silently come from the local pool, which is the state QuoteGarden already left behind, so the swap alone would have changed nothing. QuoteSlate stays first and takes over the moment it is reachable. Its rate-limit notice arrives shaped like a real quote attributed to `zenquotes.io`, and is treated as the failure it is rather than shown as the morning's inspiration.
- Retained unchanged: 6s timeout, Zod validation of every response, 3 attempt ceiling, 30 day duplicate avoidance, curated local fallback, no client-side provider calls.
- Sabbath data-selection bugs fixed. `prepareSabbath` hardcoded a verse-only fallback pool, ignoring the saved `quoteSourcePref`, and never consulted a quote pinned to that date although Today did. Both now call one resolver, `lib/inspiration/rest-quote.ts`, over one pure rule, `pickRestDayQuote`. The Sabbath prose, its fallback notification and the `sabbath` payload shape are untouched: only which quote travels with the message changed.
- The Today page no longer carries its own copy of the rest-day sequence, which is what had drifted from the worker in the first place.
- Morning contract unchanged in shape: `dailyInspiration` stays, the legacy `quote` field stays for the live workflow, and `evening_summary` still has neither.
- Verification: typecheck clean, lint clean, 653/653 unit tests pass, `pnpm build` compiles.

### 2026-08-27: Canonical Daily Inspiration
- New `daily_inspirations` ledger (migration `0019`): one row per (user, local date), holding `type`, `text`, `source`, `translation`, `provider`. Purely additive: `CREATE TYPE inspiration_type` plus `CREATE TABLE`, no `ALTER` against any existing table, so existing rows and the `daily_quotes` pool are untouched.
- The unique constraint on `(user_id, local_date)` IS the concurrency guarantee. `insertIfAbsent` uses `ON CONFLICT DO NOTHING` and re-reads on an empty `returning()`, so when the worker materializing a 06:00 morning job and the owner opening Today both miss and both fetch, one wins and the other adopts the winner. Reading first and then inserting would let both write.
- `lib/inspiration/resolve.ts` is deliberately free of `server-only` and of Drizzle: the store is an interface, so one-record-per-day, the retry ceiling, freshness and fallback are tested against an in-memory store rather than Postgres. `db/repositories/inspirations.ts` supplies the real store.
- Providers: QuoteGarden V3 and bible-api.com (WEB), both keyless, both normalized into one shape and validated with Zod. Every failure mode (timeout, non-200, malformed body, over-long content, a repeat inside 30 days) is treated the same by the caller: retry, up to 3 attempts, then the curated local pool in `lib/inspiration/fallback.ts`. Over-long content is rejected and refetched, never truncated, because cutting scripture or an aphorism changes what it says.
- One server entry point, `lib/inspiration/daily.ts`, used by both the Today page and the worker. The audit found the older quote path had drifted because the same selection was written out at four call sites and one was later changed alone; this is the structural answer to that.
- `MorningBriefPayload` gains `dailyInspiration`. The existing `quote` field is unchanged so the live workflow does not break. `evening_summary` deliberately does not carry it, and a test locks that.
- Today renders `DailyInspirationCard` in the slot that was reserved under Momentum, so nothing else on the page moved. The now-dead `DailyQuoteCard` was removed and the rest-day quote type moved beside `SabbathBanner`, which still reads the themed `daily_quotes` pool.
- Sabbath is untouched: the rest-day banner keeps its themed quote and the `sabbath` job payload is unchanged. The resolver is skipped entirely on a rest day rather than spending a provider call on something the page will not render.
- Known, NOT fixed here: the Sabbath divergence reported in the audit. `prepareSabbath` still hardcodes `["verse"]` for its fallback pool instead of honouring `quoteSourcePref`, and still ignores a pinned quote that the Today page honours. It sits inside notification composition, which this task fenced off.
- Verification: typecheck clean, lint clean, 632/632 unit tests pass, `pnpm build` compiles.

### 2026-08-22: Password reset, welcome email, and first-login onboarding
- **Password reset uses Better Auth's native primitive, not a new one.** 1.6.23 already issues a random token into `verification` keyed `reset-password:<token>`, consumes it on use so it cannot be replayed, enforces the expiry, and answers `/request-password-reset` with the SAME body for a known and an unknown address (with a dummy lookup so the timing matches). `lib/auth.ts` now supplies `sendResetPassword`, `resetPasswordTokenExpiresIn` (1 hour) and `revokeSessionsOnPasswordReset: true`. Writing a custom reset system could only have been weaker.
- New pages `/forgot-password` and `/reset-password`, both added to the proxy's public paths: someone resetting a password cannot sign in, so redirecting them to /login would make the emailed link bounce to the screen they are locked out of. "Forgot password?" sits under the password field on sign-in.
- **Email leaves through n8n only.** New `lib/email-automation/n8n-email-events.ts`: a typed sender built on the same shape as `lib/push/web-push.ts` (factory over injected dependencies, `readConfig(env)` with a process.env default), so tests drive the real code with a fake transport. It NEVER throws: the reset path must answer identically whether or not an account exists, and the sign-up path must not fail an account that already exists, so failure is a return value.
- Failures never log the reset URL or the token, only `eventType` and `eventId`. A fetch error message can contain the whole request URL, and a log is durable in a way a single-use credential must not be.
- **Welcome email fires on account creation only**, from the one place that can tell: a successful sign-up POST that returned a user, in `app/api/auth/[...all]/route.ts`. A hook on session creation would have sent one on every login and every device.
- Welcome dedupe reuses the `claimInvite` shape rather than new infrastructure: a conditional upsert that only writes while `welcome_email_sent_at` is null, so exactly one caller wins. The claim is taken before dispatch and released if the handoff fails, so a crash cannot produce a second email and a failure cannot mark someone welcomed forever.
- **First-login onboarding needed no new column.** `user_settings.onboarding_completed_at` has existed since migration 0000 and was completely dormant. Three steps, mounted from the app shell when the timestamp is null; the last step hands off to the existing Settings notification flow rather than reproducing it. Every exit persists, including "Maybe later" and the close control: a popup that returns until it is endured is a worse popup.
- Added an `id` prop to `SettingsSection` and `id="notifications"` to that group, because the CTA needs an anchor and no section had one. `/iphone/setup` is deliberately NOT the target: without a pairing fragment it dead-ends and sends the reader back to Settings.
- Migration 0018: adds nullable `welcome_email_sent_at`, and backfills BOTH milestone columns for existing rows. Without the backfill, applying it would greet every established account with an onboarding tour and queue a retroactive welcome email. New accounts keep NULL and get both.
- Rate limiting is Better Auth's own: `/request-password-reset` at 3 per 60 seconds, on by default in production. The default counter is per serverless instance, which is documented in the contract doc as a known limitation rather than papered over.
- New doc `docs/n8n-email-contracts.md` with both payloads, the bearer header, token-secrecy rules for the workflow, idempotency, and the no-retry rationale. No real secrets.
- New env vars, server only: `N8N_PASSWORD_RESET_WEBHOOK_URL`, `N8N_WELCOME_EMAIL_WEBHOOK_URL`, `N8N_EMAIL_WEBHOOK_SECRET`. Verified absent from all 54 client bundles after a production build.
- Verification: typecheck clean, lint clean, 575/575 unit tests pass (60 new), `pnpm build` compiles with both new routes. Migration 0018 is generated and NOT applied; the owner applies it by hand.
