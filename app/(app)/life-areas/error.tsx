...
/**
 * Error boundary for the Life Areas route (e.g. the database is unreachable).
 * Never a blank panel: it explains the failure and offers a retry.
 */
export default function LifeAreasError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Life Areas route error", error);
  }, [error]);
  ...
  return (
    <div className="card-shadow flex flex-col items-center justify-center rounded-xl border border-outline-variant bg-surface-container-lowest px-6 py-16 text-center">
      ...
      <h2 className="text-headline-md text-on-surface">We couldn&apos;t load your life areas</h2>
